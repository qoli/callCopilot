...
if (!apiKey) {
  throw new Error("NVIDIA_DEEPSEEK_API_KEY is required");
}

const upstreamBaseUrl = (args["upstream-base-url"] ?? "https://integrate.api.nvidia.com/v1").replace(/\/+$/, "");
...
const server = createServer(async (req, res) => {
  try {
    ...
    console.error("[nvidia-deepseek-proxy] error", error);
    if (!res.headersSent) {
...
server.listen(port, host, () => {
  ...
  console.error(`[nvidia-deepseek-proxy] listening on ${baseUrl}`);
});
...
async function handleRequest(req, res) {
  ...
  const isChatCompletions = isOpenAIChatCompletionsPath(req.url);

  if (req.method !== "GET" && rawBody && isChatCompletions) {
  ...
  if (!isChatCompletions || !upstreamResponse.ok) {
...
function isOpenAIChatCompletionsPath(url) {
  const pathname = url.split("?", 1)[0].toLowerCase();
  return pathname.endsWith("/v1/chat/completions") || pathname.endsWith("/chat/completions");
}
